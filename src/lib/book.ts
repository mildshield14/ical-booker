...
export async function createBooking(
    ...
    const icsValue: string = await new Promise((res, rej) => {
        createEvent({ start: toArr(start), end: toArr(end), title,
    ...
    const ok = await fetch(url, {
        ...
        headers: {
            Authorization: 'Basic ' + Buffer.from(
                `${creds.username}:${creds.password}`).toString('base64'),
...
function toArr(iso: string) {
    ...
    return [
        ...
        d.getUTCHours(),
        d.getUTCMinutes(),
    ] as [number, number, number, number, number];   // DateTime type
}
