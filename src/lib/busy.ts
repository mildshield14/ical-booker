...
export async function getBusyEvents(
  ...
  <C:calendar-query xmlns:C="urn:ietf:params:xml:ns:caldav" xmlns:D="DAV:">
    ...
    const bodies = await Promise.all(calendars.map(({ url }) =>
        fetch(url, {
            method : 'REPORT',
    ...
    for (const xml of bodies) {
        for (const [, ics] of xml.matchAll(/<calendar-data[^>]*>(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?<\/calendar-data>/gi)) {
            ...
            for (const ev of Object.values(parsed) as any[]) {
                ...
                rule.between(new Date(startISO), new Date(endISO), true)
                    .forEach(dt => {
